 ...
 */
 ...
 namespace Udjat {
	...
	std::shared_ptr<Abstract::Agent> HTTP::Agent::Factory::AgentFactory(const Abstract::Object &, const XML::Node &node) const {
		return make_shared<HTTP::Agent>(node);
	}

	HTTP::Agent::Agent(const XML::Node &node) 
		: 	Udjat::Agent<unsigned int>{node}, Udjat::URL{node,"url"} {
	}
